...
module.exports.run = async (bot, message, args) => {
    try {
        if (message.author.id !== 768788268053430292) throw new Error('Команда доступна только разработчику');
        let member = message.mentions.users.first() || message.member;
        if (users[member.user.id]) throw new Error('ERROR_USER_ALREADY_HAS_ACCOUNT');
